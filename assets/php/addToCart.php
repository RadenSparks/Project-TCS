<?php    
    ...
    $check = mysqli_query($conn, "SELECT top 1 c.email, ci.gameid FROM cart c JOIN cartitem ci ON c.cartid = ci.cartid WHERE status = 1 and c.email = '".$_SESSION['email']."' and ci.gameid = '".$_GET['id']."'") or die(mysqli_error($conn));
