...
<?php
    if (isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['displayname'])
        && isset($_POST['email']) && isset($_POST['pass'])) 
    {
        ...
        $checkEmail = mysqli_query($conn, "SELECT email, password FROM accounts WHERE email = '$email'");
        ...
        {
            $addEmail = mysqli_query($conn, "INSERT INTO accounts (email, password, firstname, lastname, displayname) VALUES ('".$email."', '".$pass."', '".$firstName."', '".$lastName."', '".$displayName."')");
            if ($addEmail) 
            {
                echo '<script type="text/javascript">window.location="/Project-TCS/index.php?site=login"</script>';
                exit();
            }
            else {
                echo '
