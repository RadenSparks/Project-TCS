...
if (isset($_POST['editProduct_submit'])) {
    ...
    $productId = intval($_POST['gameid']); // Chắc chắn rằng productId là một số nguyên
    $productGenre = htmlspecialchars($_POST['genre']);
    $productPrice = floatval($_POST['price']); // Chắc chắn rằng productPrice là một số thực
    $productSale = isset($_POST['sale']) ? intval($_POST['sale']) : null; // Chắc chắn rằng productSale là một số nguyên hoặc null
    ...
    $success = updateProduct($productName, $productId, $productGenre, $productPrice, $productSale, $productDeve, $productPub, $productSummary, $productImg1);
...
?>
<?php
include_once 'prod_edit_main.php';
?>
                       
<?php
// Hàm để cập nhật thông tin sản phẩm trong cơ sở dữ liệu
function updateProduct($productName, $productId, $productGenre, $productPrice, $productSale, $productDeve, $productPub, $productSummary, $productImg1)
{
    $conn = conndb();
    $sql = "UPDATE game 
