       ...
       // Di chuyển file tải lên vào thư mục lưu trữ và kiểm tra kết quả
        ...
        if ($_FILES['image4']['error'] === UPLOAD_ERR_OK && $_FILES['image5']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($uploadSuccess) {
                ...
                echo "<script>alert('Thêm thành công'); window.location.href = '?pg=product_list';</script>";
            } else {
            ...
            $productIconPath, $productLogoPath, $productDeve, $productPub, $conn);
            echo "<script>alert('Thêm thành công (không có ảnh 4, 5)'); window.location.href = '?pg=product_list';</script>";
        }
    } else {
